...
function joinGame() {
  const players = [];
  // join websocket channel to broadcast presence
  Pusher.logToConsole = process.env.NODE_ENV !== "production";
  const pusher = new Pusher(process.env.PUSHER_KEY, {
    cluster: "us2",
    forceTLS: true
  });
  const wsChannel = pusher.subscribe(`${gameName}-scrabble-registration`);
  wsChannel.bind("message", function(data) {
    alert(JSON.stringify(data));
  });
  wsChannel.bind("introduction", function(data) {});
}
